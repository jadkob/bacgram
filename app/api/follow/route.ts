...
export async function POST(req: Request) {
    try {
        ...
        if (user1?.followers.includes(user2?._id as any)) return new Response("Already followed", { status: 409})
        user1?.followers.push(user2?._id as any)
        user2?.following.push(user1?._id as any)
